...
智能合约不支持浮点运算, 除法得到的也是整除的结果(`5`/`2`=`2`)
...
`abi.decode(bytes memory encodedData, (...)) returns (...)` abi 解码, e.g.
...
`abi.encodeWithSelector(bytes4 selector, ...) returns (bytes memory)` abi 编码, 并在前面加上 4 个字节的`function selector`
