...
范围从`uint8`到`uint256`, 从`int8`到`int256`
其中`uint`/`int`是`uint256`/`int256`的别名

可用操作符: `<=`,`<`,`==`,`!=`,`>=`,`>`,`&`,`|`,`^`,`~`,`+`,`-`,`*`,`/`,`%`,`**`,`<<`,`>>`

智能合约不支持浮点运算, 除法得到的也是整除的结果

#### `Address`地址类型

地址类型持有一个 20 字节的数字(和以太坊地址长度一样)

成员及方法:

- `balance` 地址当前的余额
- `transfer(uint256 amount)` 向该地址转账(接受转账的地址必须被`payable`修饰)
- `send`,`call`,`callcode`,`delegatecall` 底层函数调用, 可能引起安全问题, 不建议使用, 具体使用参考文档
...
由于 EVM 特殊的存储结构, mapping 无法遍历(但有些第三方库实现了这个功能, 本质是一个 mapping 加上一个 array)
...
`abi.decode(bytes memory encodedData, (...)) returns (...)` abi 解码. Example: `(uint a, uint[2] memory b, bytes memory c) = abi.decode(data, (uint, uint[2], bytes))`

`abi.encode(...) returns (bytes memory)` abi 编码, 一般用于跨合约调用

`abi.encodePacked(...) returns (bytes memory)` abi 编码(非标准模式, 这样编码出来更节省空间)

`abi.encodeWithSelector(bytes4 selector, ...) returns (bytes memory)` abi 编码, 并且在前面加上 4 个字节的`function selector`
...
`revert(string memory reason)` 回滚并展示错误信息
...
声明合约使用`contract`关键字, Solidity 中的合约就是其他语言中的类
...
contract SimpleStorageInterface {
    function func1() external;
    function func2() external returns(uint256, uint256);
}
```
