...
- `balance` 地址当前的余额
- `transfer(uint256 amount)` 向该地址转账
...
contract SimpleAuction {
    event HighestBidIncreased(address indexed bidder, uint anonymous amount); // Event
...
- `indexed`关键字代表以太坊会为事件中的这个字段创建索引, 这可以提高查询日志的效率
- `anonymous`关键字代表以太坊不会为事件中的这个字段创建索引(默认)
